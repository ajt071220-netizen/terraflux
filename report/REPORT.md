...
<!-- 【你来写】全文主菜，约 530–580 词。
先用一段"设计哲学"开场（2.0，约 80–100 词），再三小节各约 150 词，
固定写法：这机制是什么（2 句）→ 传统棋类为什么从不这样做（1 句）→
它改变了什么（1–2 句）。规则细节全部查 report/写作底稿.md §1.1 与 §6。 -->

<!-- 2.0 设计哲学段（开场）：回答"地形效果为什么只有隆起/下陷/填平三种"。
核心概念＝价值镜像检验：追逃结构里，凡是影响移动效率的效果都是单边
武器（传送/加速纯偏白、冻结/减速纯偏黑）；唯有高度（黑攻白防）与通行性
（堵路双刃剑）对双方价值镜像。词汇表小不是缺想象力，是淘汰后的幸存。
参考句式："The terrain vocabulary is small by design: any effect that
touches movement efficiency is a one-sided weapon in a pursuit game —
...
> [2.0 design philosophy to be written]
...
未来工作挑 2–3 条：多种子复核；强策略下的尺寸扫描；LLM 智能体作为
第三类强度样本；以及机制向的开放问题（重点推荐这条，呼应 §2.0）：
候选地形机制清单已就绪——更高档位（±2 柱）、退潮格（填平数手后
自动复原）、地形自主演化（下陷格缓慢回弹）——它们都自称能通过价值
镜像检验，但每一个都必须经 RL 裁判流水线重新测量公平性才敢入列；
"新机制想法 → 实现 → RL 测量 → 数据裁决"是一条可持续产出的研究
流水线。这告诉读者：你看到的问题比解决的多。
